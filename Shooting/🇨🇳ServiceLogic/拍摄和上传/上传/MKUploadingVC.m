...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor colorWithPatternImage:KBuddleIMG(@"⚽️PicResource", @"Others", nil,@"nodata")];
...
//                    NSLog(@"KKK = %@", arg);
                    if ([arg isKindOfClass:UIImage.class]) {
                        ...
                        [self.choosePicBtn setImage:[UIImage addImage:[UIImage cropSquareImage:self.imgData]
                                                            withImage:KBuddleIMG(@"⚽️PicResource",@"拍摄*上传", nil,@"播放")
                                                    image2Coefficient:3]
...
- (void)deleteButtonRemoveSelf:(LGiOSBtn *_Nonnull)button{
    [button setImage:KBuddleIMG(@"⚽️PicResource",@"Others", nil,@"加号")
            forState:UIControlStateNormal];
